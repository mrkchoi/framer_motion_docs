...
function Mesh() {
  ...
  useFrame(({ clock }) => {
    ...
    meshRef.current.material.uniforms.uMultiplier.value = multiplier;
  });

  return (
    <mesh ref={meshRef} scale={[300, 300, 300]}>
      <icosahedronGeometry args={[1, 100, 100]} />
      <vision02Material key={Vision02Material.key} side={THREE.DoubleSide} />
    </mesh>
  );
}
...
const Vision02Material = shaderMaterial(
  ...
  */
  ...
  void main() {
    vec3 coords = normal;
    coords.y += uTime * uSpeed;
    vec3 noisePattern = vec3(noise(coords * uMultiplier));
